/* Bottleneck Bandwidth and RTT (BBR2) congestion control
...
struct bbr {
	...
	u32	max_rtt_us;	        /* min RTT in min_rtt_win_sec window */
	u32	rtt_us;	        /* min RTT in min_rtt_win_sec window */
	u32	min_rtt_stamp;	        /* timestamp of min_rtt_us */
	u32	probe_rtt_done_stamp;   /* end time for BBR_PROBE_RTT mode */
	struct minmax bw;	/* Max recent delivery rate in pkts/uS << 24 */
	struct minmax max_rtt;	/* Max rtt with win-filtered */
...
static const int bbr_pacing_gain[] = {
	...
	BBR_UNIT * 5 / 4, BBR_UNIT * 5 / 4, BBR_UNIT * 5 / 4	/* without creating excess queue... */
};
 ...
 */
static void bbr_set_pacing_rate(struct sock *sk, const struct rate_sample *rs, u32 bw, int gain)
{
	...
	rate = bbr_rate_bytes_per_sec(sk, rate, tgain);
	rate = min_t(u64, rate, sk->sk_max_pacing_rate);
	if (rs) {
		printk("loss:%d  st:%d  mode:%d  gain:%d  tgain:%d  pacing:%llx  cwnd:%d\n",rs->losses, state, bbr->mode, gain, tgain, rate, tp->snd_cwnd);
	}
...
static void bbr_cwnd_event(struct sock *sk, enum tcp_ca_event event)
{
	...
	if (event == CA_EVENT_TX_START && tp->app_limited) {
		...
		if (bbr->mode == BBR_PROBE_BW)
			bbr_set_pacing_rate(sk, NULL, bbr_bw(sk), BBR_UNIT);
	}
}
			 ...
			 u32 acked, u32 bw, int gain)
{
	...
	printk("mode:%d  st:%d  cwnd:%d  pacing:%u\n", bbr->mode, inet_csk(sk)->icsk_ca_state, tp->snd_cwnd, sk->sk_pacing_rate);
	if (!acked)
		return;
...
static void bbr_check_drain(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (bbr->mode == BBR_STARTUP && bbr_full_bw_reached(sk)) {
		...
		bbr->cwnd_gain = BBR_UNIT;//bbr_high_gain;	/* maintain cwnd */
...
static void bbr_update_min_rtt(struct sock *sk, const struct rate_sample *rs)
{
	...
	else if (rs->rtt_us >= 0 && !filter_expired) {
		bbr->max_rtt_us = rs->rtt_us;
	}
	bbr->rtt_us = rs->rtt_us;
...
static void bbr_main(struct sock *sk, const struct rate_sample *rs)
{
	...
	bw = bbr_bw(sk);
	bbr_set_pacing_rate(sk, rs, bw, bbr->pacing_gain);
...
static void bbr_init(struct sock *sk)
{
	...
	bbr->max_rtt_us = tcp_min_rtt(tp);
	bbr->rtt_us = tcp_min_rtt(tp);
	bbr->min_rtt_stamp = tcp_time_stamp;

	minmax_reset(&bbr->bw, bbr->rtt_cnt, 0);  /* init max bw to 0 */
	minmax_reset(&bbr->max_rtt, bbr->rtt_cnt, 0);  /* init max bw to 0 */

	/* Initialize pacing rate to: high_gain * init_cwnd / RTT. */
	bw = (u64)tp->snd_cwnd * BW_UNIT;
	do_div(bw, (tp->srtt_us >> 3) ? : USEC_PER_MSEC);
	sk->sk_pacing_rate = 0;		/* force an update of sk_pacing_rate */
	bbr_set_pacing_rate(sk, NULL, bw, bbr_high_gain);
...
static void bbr_set_state(struct sock *sk, u8 new_state)
{
	...
	if (new_state == TCP_CA_Loss) {
		...
		printk("mode:%d  cwnd:%d\n", bbr->mode, tp->snd_cwnd);
